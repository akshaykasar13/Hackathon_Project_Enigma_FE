...
export async function GET(request: NextRequest) {
  ...
  const url = `${base}${pathPrefix ? `/${pathPrefix}` : ''}/observability/ai-metrics/embedding-calls${query ? `?${query}` : ''}`
  try {
    ...
    console.error('[api/observability/ai-metrics/embedding-calls]', url, err)
    return NextResponse.json(

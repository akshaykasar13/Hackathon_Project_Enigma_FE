...
export async function GET(request: NextRequest) {
  ...
  const url = `${base}${pathPrefix ? `/${pathPrefix}` : ''}/memory/semantic${query ? `?${query}` : ''}`
  try {
    ...
    if (!res.ok) {
      console.error('[api/memory/semantic GET]', res.status, url, data)
    ...
    console.error('[api/memory/semantic GET]', url, err)
    return NextResponse.json(
      {

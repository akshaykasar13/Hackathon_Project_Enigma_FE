...
export async function GET(request: NextRequest) {
  ...
  const url = `${base}${pathPrefix ? `/${pathPrefix}` : ''}/memory/episodic${query ? `?${query}` : ''}`
  try {
    ...
    if (!res.ok) {
      console.error('[api/memory/episodic GET]', res.status, url, data)
    ...
    console.error('[api/memory/episodic GET]', url, err)
    return NextResponse.json(
      {
